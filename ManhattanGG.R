...
parser <- argparse::ArgumentParser(description=":: ManhattanGG v.0.0.5 alpha; by Beomsu Kim. ::", formatter_class="argparse.ArgumentDefaultsHelpFormatter")
...
argnames_ordered <- c("assoc", "metal", "out", "paper_material", "qq", "qq_only", "qq_lambda", "pdf", "Rdata", "chr", "pos", "snp", "pval", "y_lim", "y_breaks", "y_ceiling", "point_size", "text_size", "deco", "deco_color", "deco_label", "deco_label_color", "highlight_color", "highlight_flank", "trait", "trait_color", "suggestiveline", "genomewideline", "line_type", "text_angle", "title")
argvals_default <- c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, "CHR", "BP", "SNP", "P", FALSE, FALSE, FALSE, 1.2, 3.5, FALSE, FALSE, FALSE, FALSE, FALSE, 1000, FALSE, FALSE, 1e-05, 5e-08, "solid", 0, "")
...
Formatting_inputs <- function(assocf, decof=FALSE, metal=FALSE, chr="CHR", bp="BP", snp="SNP", p="P", deco.color=FALSE, deco.label=FALSE, deco.label.color=FALSE, trait=FALSE, qq_only=FALSE){
    ...
    if(metal){
        ...
        if(!all(c(chr, bp) %in% colnames(df)) & !(qq_only)){
            ...
            df[, c(chr, bp, "tmp1", "tmp2")] = as.data.table(stringr::str_split_fixed(df[, "MarkerName"], ":", 4))
            df <- data.frame(df)
        }
    }
...
manhattan <- function(df, metal=FALSE, chr="CHR", bp="BP", snp="SNP", p="P", y.lim=FALSE, y.breaks=FALSE, y.ceiling=FALSE, p.size=2, text.size=4,
                      ...
                      l.type="solid", title="", text_angle=0, ManhattanGG=FALSE){
    ...
    if(n_chr==1){
        ...
        xlabel = "Chromosome"
        labs = unique(df$chr)
        snpcolors = rep(c("gray20", "gray50"), length.out=n_chr)
    }
...
#      scale_y_continuous(name=expression(paste(-log[10], " (", italic(P), ")", collapse="")), breaks=ybreaks, expand=c(0,0)) +
    ...
    # deco label
    if(is.data.frame(deco)){
        if(is.character(deco.label)){
            label.parse = ifelse(any(grepl("italic", df_deco$label)), TRUE, FALSE)
            if(text_angle==0){
                plot <- plot + ggrepel::geom_text_repel(data=df_label, aes(x=pos, y=logP, label=label, colour=factor(labelcolor)), size=text.size, point.padding=unit(0, "lines"), nudge_y=yMax/50, vjust=0, parse=label.parse, angle=text_angle, show.legend=FALSE)
            }else{
                plot <- plot + ggrepel::geom_text_repel(data=df_label, aes(x=pos, y=logP, label=label, colour=factor(labelcolor)), size=text.size, point.padding=unit(0, "lines"), nudge_y=yMax/50, vjust=0, parse=label.parse, angle=text_angle, show.legend=FALSE)
            }
        }
...
if(opt.qq_only){
    ...
    if(opt.paper_material==FALSE){
        ...
        suppressWarnings(manplot <- manhattan(df=df, meta=opt.metal, chr="chr", bp="bp", snp="snp", p="p", y.lim=opt.y_lim, y.breaks=opt.y_breaks, y.ceiling=opt.y_ceiling, p.size=opt.point_size, text.size=opt.text_size,
                                              ...
                                              l.type=opt.line_type, title=opt.title, text_angle=opt.text_angle, ManhattanGG=TRUE))
        ggsave(paste(manpref, opt.pdf, sep="."), plot=manplot$plot, dpi=300, width=30, height=15, units="cm")
        ...
        ggsave(paste(manpref, "point", opt.pdf, sep="."), plot=plotpoint, dpi=300, width=30, height=15, units="cm")
        ...
        suppressWarnings(manplot.label <- manhattan(df=df_labelplot, meta=opt.metal, chr="chr", bp="bp", snp="snp", p="p", y.lim=opt.y_lim, y.breaks=opt.y_breaks, y.ceiling=opt.y_ceiling, p.size=opt.point_size, text.size=opt.text_size,
                                                    ...
                                                    highlight.color=opt.highlight_color, highlight.flank=opt.highlight_flank, suggestiveline=opt.suggestiveline, genomewideline=opt.genomewideline,
                                                    l.type=opt.line_type, title=opt.title, text_angle=opt.text_angle, ManhattanGG=TRUE))
        ggsave(paste(manpref, "label.pdf", sep="."), plot=manplot.label$plot, dpi=300, width=30, height=15, units="cm")
